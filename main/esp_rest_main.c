...
static void client_post_function()
{
    esp_http_client_config_t config_post = {
        .url = "http://192.168.0.108:3000/api/v1/students",
...
void tag_handler(char *sn)
{ // serial number is always 5 bytes long
    // snprintf(bufferTag, sizeof(bufferTag), "%x%x%x%x%x", sn[0], sn[1], sn[2], sn[3], sn[4]);
    snprintf(buffer, sizeof(buffer), "{\"RfidTag\": \"%x%x%x%x%x\"}", sn[0], sn[1], sn[2], sn[3], sn[4]);
